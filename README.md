...
- Reset posts thread a reply with the one-line detail and a single link to the AI StopWatch item. One consistent destination, it credits the newsroom doing the reading, and it is one URL rather than the four that drew a 403 from X. The primary sources stay in `incidents.yaml`, which is the evidence record; entries added by hand fall back to their own source URL. Tier 2 additions thread as replies too.
